  ...
  fetchConfig: async () => {
    try {
      ...
      if (error && error.code !== 'PGRST116') {
        console.warn('kv_store table or loyalty_config key missing, using fallback config:', error.message);
      }
  ...
  updateConfig: async (newConfig: LoyaltyConfig) => {
    ...
    try {
      const { error } = await supabase
        .from('kv_store_7d96d7c8')
        .upsert({ key: 'loyalty_config', value: newConfig }, { onConflict: 'key' });

      if (error) {
        console.warn('Could not save loyalty_config in Supabase kv_store, saved in memory/localStorage:', error.message);
